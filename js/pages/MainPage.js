...
export default class MainPage {
        ...
        this._postCreateFormEl.addEventListener('submit', evt => {
            ...
            this._context.post('/posts', JSON.stringify(data), {'Content-Type': 'application/json'},
                text => {
                    ...
                    this.clean();
                    this.loadMorePosts(0, this._postsCount);
    ...
    loadMorePosts(from, another) {
        const str = "?from=" + from + "&another=" + another;
    ...
    loadPosts(posts) {
        for (const post of posts) {
            ...
            if (post.media !== null) {
                if (post.media.endsWith('.png') || post.media.endsWith('.jpg')) {
                    postMedia += `<img src="${this._context.mediaUrl()}/${post.media}" class="card-img-top" alt="...">`;
        ...
        if(posts.length === 0){
            const nothingToShowEl = document.createElement('div');
            nothingToShowEl.className = 'col-4';
            nothingToShowEl.innerHTML = `
            <div class="card mt-2">
                <div class="card-body">
            ...
            this._postsContainerEl.appendChild(nothingToShowEl);
            return;
        }
        ...
        loadPostEl.innerHTML = `
            <div class="card mt-2">
                <div class="card-body">
                    <div class="col text-center">
                        <a href="#" data-action="upload-posts" class="btn btn-sm btn-primary">Load more posts</a>
                    </div>
                </div>
            </div>`;
    ...
    newPostsDetected() {
        const str = "?last=" + this._firstPostId;
        this._context.get("/posts" + str, {},
            text => {
